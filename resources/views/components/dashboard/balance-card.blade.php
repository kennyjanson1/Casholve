{{-- resources/views/components/dashboard/balance-card.blade.php --}}
...
@php
    ...
    // Calculate income
    $currentIncome = auth()->user()->transactions()
        ->where('type', 'income')
        ->whereBetween('date', [$startOfMonth, $endOfMonth])
        ->sum('amount');
    
    // Calculate expenses
    $currentExpenses = auth()->user()->transactions()
        ->where('type', 'expense')
        ->whereBetween('date', [$startOfMonth, $endOfMonth])
        ->sum('amount');
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            ...
            <div class="flex items-baseline gap-3 mb-1">
                ...
                @if($balanceChange != 0)
                    <span class="flex items-center gap-1 text-sm {{ $balanceChange >= 0 ? 'text-green-500' : 'text-red-500' }}">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">Your Balance in {{ Carbon::now()->format('F Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
                ...
                @if($expenseChange != 0)
                    <span class="flex items-center gap-1 text-xs {{ $expenseChange <= 0 ? 'text-green-500' : 'text-red-500' }}">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            @if($expenseChange <= 0)
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6"></path>
                            @else
